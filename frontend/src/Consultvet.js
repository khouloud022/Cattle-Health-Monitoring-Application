...
import Nav from './Nav4'; // Supposant que vous avez ce composant
...
function App() {
  ...
  const [Pred, setPredictions] = useState({}); 

 
  useEffect(() => {
    const savedPredictions = localStorage.getItem('Pred');
    if (savedPredictions) {
  ...
  const handlePredict = async (item) => {
    try {
      const response = await fetch('http://127.0.0.1:5000/api/predict', {
        ...
        body: JSON.stringify({
          IN_ALLEYS: item.IN_ALLEYS / 100000,
          REST: item.REST / 100000,
          EAT: item.EAT / 100000,
          ACTIVITY_LEVEL: item.ACTIVITY_LEVEL / 100000,
          LPS: parseFloat(item.LPS),
      ...
      setPredictions(prevState => {
        const updatedPredictions = { ...prevState, [item.index]: data.Pred };
        localStorage.setItem('Pred', JSON.stringify(updatedPredictions)); 
        return updatedPredictions;
      });
    } catch (err) {
      console.error("Erreur de requête vers le serveur Flask :", err);
    }
  };
    ...
    <table style={styles.dataTable}>
      <thead>
        <tr>
          ...
          <th style={styles.tableHeader}>Predict Cattle Health</th>
        </tr>
      </thead>
      <tbody>
        {dataItems.map((item, idx) => (
          <tr key={idx} style={styles.tableRow}>
            ...
            <td style={styles.tableCell}>
              {/* Si la prédiction est disponible, afficher la prédiction à la place du bouton */}
              {Pred[item.index] ? (
                <div style={styles.predictionResult}>
                  Prediction: {Pred[item.index]}
                </div>
              ) : (
                <button
      ...
      <div className="main">
        <div style={styles.appContainer}>
          ...
          <div style={styles.statusIndicator}>
            <p>Total data in the contract: {dataLength}</p>
          </div>
        </div>
      </div>
    </div>
